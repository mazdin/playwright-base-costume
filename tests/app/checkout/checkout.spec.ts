...
test.describe("Checkout Feature", () => {
    ...
    test("CHK-03 | Input spasi saja pada form", async ({ checkoutPage }) => {
        ...
        await description(
            "Verifikasi perilaku form saat semua field diisi spasi (SauceDemo tidak memvalidasi whitespace — lanjut ke overview).",
        );
        await tag("edge-case");
    ...
    test("CHK-04 | Verifikasi perhitungan pajak (8%)", async ({ checkoutPage }) => {
        ...
        await severity("normal");
        await description(
            "Verifikasi nilai Tax = 8% dari subtotal dan Total = Subtotal + Tax pada halaman Checkout Overview.",
        );
        ...
        await step("Verifikasi Tax = 8% dari Item total dan Total = Subtotal + Tax", async () => {
            await checkoutPage.performVerifyTax();
        });
    });

    test("CHK-05 | E2E checkout sampai order complete", async ({ checkoutPage }) => {
        await story("Complete Order");
        await severity("critical");
        await description("Verifikasi alur penuh: isi data → overview → Finish → halaman 'Thank you for your order!'.");
        await tag("smoke");
        await tag("e2e");

        await step("Isi data valid dan lanjut ke overview", async () => {
            await checkoutPage.performFillInfo(CHECKOUT_INFO.valid);
        });
        await step("Klik Finish dan verifikasi order selesai", async () => {
            await checkoutPage.performFinishOrder();
        });
    });
});
